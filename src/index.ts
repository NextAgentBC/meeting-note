 ...
 * object, so switch thinking off and leave the whole budget for the answer. `tuned` additionally lowers
 * temperature and top_p and adds a small frequency penalty for the one-shot final merge, matching the
 * fork's measured settings there; the segment call keeps the template's plain temperature, since the
 * segment prompt already runs once per section and the extra tuning is unproven at that scale. Other
 * models may reject unrecognised params, so all of this stays scoped to GLM.
 */
function modelOptions(model: string, tuned = false): Record<string, unknown> {
